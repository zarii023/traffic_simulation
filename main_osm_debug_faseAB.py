...
def look_at(eye, center, up=(0, 1, 0)):
    ...
    f, s, u = normalize(center - eye), None, None
    s = normalize(np.cross(f, up)); u = np.cross(s, f)
    M, T = np.eye(4, dtype=np.float32), np.eye(4, dtype=np.float32)
    M[0,:3], M[1,:3], M[2,:3], T[:3,3] = s, u, -f, -eye
    return M @ T
...
def load_osm_highways(osm_path: Path):
    ...
    ways = [[int(nd.attrib["ref"]) for nd in way.findall("nd")]
...
def build_road_mesh(nodes_xy, ways, width_m=8.0):
    half_w = width_m / 2.0
    vertices, indices, base_index, total_segments = [], [], 0, 0
    for nds in ways:
        pts = [nodes_xy[n] for n in nds if n in nodes_xy]
        if len(pts) < 2: continue
        for i in range(len(pts) - 1):
            (x0, z0), (x1, z1) = pts[i], pts[i + 1]
            dx, dz, seg_len = (x1 - x0), (z1 - z0), math.hypot(x1 - x0, z1 - z0)
            if seg_len < 1e-3: continue
            nx, nz = (-dz / seg_len, dx / seg_len)
            v0, v1, v2, v3 = (x0 + nx * half_w, 0, z0 + nz * half_w), (x0 - nx * half_w, 0, z0 - nz * half_w), \
                             (x1 + nx * half_w, 0, z1 + nz * half_w), (x1 - nx * half_w, 0, z1 - nz * half_w)
            vertices += [*v0, *v1, *v2, *v3]
            indices += [base_index, base_index + 1, base_index + 2, base_index + 2, base_index + 1, base_index + 3]
            base_index += 4; total_segments += 1
    return np.array(vertices, np.float32), np.array(indices, np.uint32), total_segments
...
def parse_buildings_from_osm(osm_path: Path, transformer):
    ...
    for way in root.findall("way"):
        ...
        # descartar polígonos degenerados
...
def ear_clip_triangulate(poly_closed):
    ...
    n, V, tris = len(P), list(range(len(P))), []
    if n < 3: return []
    ...
    while len(V) > 2:
        ...
        for k in range(len(V)):
            ...
            ear_found = True; tris += [i0, i1, i2]; V.pop(k); break
...
def extrude_building_mesh(poly_closed, h):
    ...
    inds += [roof_offset + i for i in ear_clip_triangulate(poly_closed)]
...
def build_vao(verts, inds):
    vao = glGenVertexArrays(1)
    glBindVertexArray(vao)
    vbo, ebo = glGenBuffers(1), glGenBuffers(1)
...
def main():
    ...
    v_road, i_road, segs = build_road_mesh(nodes_xy, ways, ROAD_WIDTH_METERS)
    buildings = parse_buildings_from_osm(OSM_FILE, transformer)
    v_build, i_build = build_all_buildings_mesh(buildings)

    vao_r, vbo_r, ebo_r = build_vao(v_road, i_road)
    vao_b = vbo_b = ebo_b = None
    if len(v_build) > 0:
        vao_b, vbo_b, ebo_b = build_vao(v_build, i_build)
    ...
    while running:
        ...
        for e in pygame.event.get():
            if e.type == QUIT: running = False
                ...
                cam_front = normalize(np.array([math.cos(yaw_r)*math.cos(pitch_r), math.sin(pitch_r), math.sin(yaw_r)*math.cos(pitch_r)], np.float32))
